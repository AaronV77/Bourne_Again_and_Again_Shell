...
int main(int argc, char * argv[], char * envp[]) {
	...
	env.env = TD_Char_Allocation(env.env_total_row_size, env.env_column_size); 
	...
	while(envp[env_iterator] != NULL) {
		if (env_storage_iterator == env.env_total_row_size) {
			...
			env.env = TD_Char_Row_Reallocation(env.env, env.env_total_row_size, temp_size, env.env_column_size);
		...
		if (!strstr(envp[env_iterator], "BASH")) {
			if (strlen(envp[env_iterator]) > env.env_column_size) {	
				...
				env.env = TD_Char_Column_Reallocation(env.env, env.env_total_row_size, temp_size);
